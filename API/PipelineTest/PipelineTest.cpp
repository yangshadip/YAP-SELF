...
void ConstructorTest()
{
	try
	{
		...
		constructor.LoadModule(L"DicomReader.dll");

		constructor.CreateProcessor(L"DicomReader", L"reader");
		constructor.SetProperty(L"reader", L"DataPath",
			L"D:\\image.dcm");
		constructor.SetProperty(L"reader", L"IsDirectory", L"false");

		constructor.CreateProcessor(L"DataTypeConvertor", L"datatypeconvertor");

		constructor.CreateProcessor(L"JpegExporter", L"jpeg_exporter");
		constructor.SetProperty(L"jpeg_exporter", L"ExportFolder", L"d:\\");

		constructor.Link(L"reader", L"datatypeconvertor");
		constructor.Link(L"datatypeconvertor", L"Float",L"jpeg_exporter",L"Input");
...
void PipelineTest()
{
	...
	auto pipeline = compiler.CompileFile(L"D:\\YAP-master\\API\\PipelineTest\\Pipelines\\FineCF.pipeline");
